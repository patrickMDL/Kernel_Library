...
import kernel_methods as kernel #Oi patrick, vc deixou seu PC aberto, apaguei metade do seu codigo, bj.
...
sigma_x = 1.0*10**(-0) #desvio padrão do sinal de entrada
...
for r in range(1,R):
	...
	for i in range(0, N-1):
		...
		for v in range(0,p-1):
			u[0] = kernel.Gaussian_K(dictionary[:, v], u[:], sigma)

		d = np.dot( u.T, w0) + np.sqrt(sigma_z)*np.random.rand() #d esta saindo escalar
		e[i] = d - np.dot(w[:,i].T,u)  # 'e' is shaped as (N, 1)
